...
public interface OrderRepository extends CrudRepository <Order,Integer>{
    ...
    @Transactional
    @Modifying
    @Query(nativeQuery = true, value = "INSERT into orders (coupon_id, discount, bill, user_id,"+" \"date\", payment_id, address_id)\n" +
                                                    "VALUES(?1, ?2, ?3, now(), ?4, ?5, ?6)")
    Integer saveOrder(Integer couponId, double discount, double bill, Integer userId, Integer paymentId,  Integer addressId);


    @Transactional
    @Modifying
    @Query(nativeQuery = true, value = "INSERT INTO public.order_item" + "(order_id, item_id, quantity, price)" +
            "VALUES(?1, ?2, ?3, ?4);")
